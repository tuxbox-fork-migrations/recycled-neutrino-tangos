	...
	int pal_addr, int flags)
{
	...
	if (supportblendingflags && flags)
		P(0x80, flags); /* blend flags... We'd really like some blending support in the drivers, to avoid punching holes in the osd */
...
CFbAccelMIPS::Mode3D CFbAccelMIPS::get3DMode()
{
	return mode3D;
}

void CFbAccelMIPS::set3DMode(Mode3D m)
{
	if (mode3D != m)
	{
		mode3D = m;
