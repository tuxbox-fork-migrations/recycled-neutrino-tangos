...
//							pthread_mutex_unlock( &cache[i].writeable );
		...
		default:
		{
			...
			if (ptr++)
			{
				if (strcasecmp(ptr, "cdr") == 0)
					f_type(fd, "audio/cdr");
				if (strcasecmp(ptr, "wav") == 0)
					f_type(fd, "audio/wave");
				if (strcasecmp(ptr, "aif") == 0)
					f_type(fd, "audio/aifc");
				if (strcasecmp(ptr, "snd") == 0)
					f_type(fd, "audio/snd");
				...
				if (strcasecmp(ptr, "mp3") == 0)
					f_type(fd, "audio/mpeg");
				if (strcasecmp(ptr, "mp2") == 0)
					f_type(fd, "audio/mpeg");
				if (strcasecmp(ptr, "mpa") == 0)
					f_type(fd, "audio/mpeg");
			}
magic_found:
			;
...
const char *f_type(FILE *stream, const char *type)
{
	...
	else
	{
		...
		else if (strstr(stream_type[i].type, type))
			return stream_type[i].type;
	}
