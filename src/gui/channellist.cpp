...
void CChannelList::paintDetails(int index)
{
	...
	if (IS_WEBTV((*chanlist)[index]->getChannelID())) {
		g_Font[SNeutrinoSettings::FONT_TYPE_CHANNELLIST]->RenderString(x+ 10, y+ height+ 5+ 2*fheight + (g_settings.channellist_foot == 0) ? 0 : fdescrheight, full_width - 30, (*chanlist)[index]->getUrl(), COL_MENUCONTENTDARK_TEXT, 0, true);
