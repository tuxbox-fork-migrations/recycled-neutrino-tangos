                                 ...
                                 const int repeat_mode, const int _zap_mode)
{
    ...
    if (!g_settings.channellist_show_numbers && g_settings.infobar_anaclock)
        show_clock(ChanInfoX + 10 + ana_clock_size/2,BoxEndY - ana_clock_size / 2 - 3, ana_clock_size / 2);
    show_dot = !show_dot;
...
bool CInfoViewer::showLivestreamInfo()
{
            ...
            cc->getEpgID() == 0 && !cc->getScriptName().empty())
    {
        ...
        if (livestreamInfo1 != _livestreamInfo1 || livestreamInfo2 != _livestreamInfo2)
        {
            ...
            showBBButtons(true /*paintFooter*/);
        }
        return true;
    }
    return false;
    ...
    else if (msg == NeutrinoMessages::EVT_ZAP_GOTAPIDS)
    {
        if ((*(t_channel_id *) data) == current_channel_id)
        {
            if (is_visible && showButtonBar)
            {
                showIcon_DD();
                showLivestreamInfo();
                showBBButtons(true /*paintFooter*/); // in case button text has changed
            }
...
*******************************************************************************/
int CInfoViewer::showChannelLogo(const t_channel_id logo_channel_id, const int logo_max_width)
{
    int res = 0;
    ...
    int logo_x = ChanInfoX + 15;
    int logo_y = ChanNameY + OFFSET_INNER_MIN;

    CComponentsChannelLogoScalable logo = CComponentsChannelLogoScalable(logo_x, logo_y, ChannelName, logo_channel_id);

    if (! logo.hasLogo())
        return res;

    logo.setWidth(min(logo.getWidth(), logo_max_width - 2*OFFSET_INNER_MID), true);
    if (logo.getHeight() > header_height - OFFSET_INNER_MIN)
        logo.setHeight(header_height - OFFSET_INNER_MIN, true);

    logo_y = logo_y + (header_height - logo.getHeight())/2;
...
bool CInfoViewer::checkBBIcon(const char * const icon, int *w, int *h)
{
...
void CInfoViewer::getBBIconInfo()
{
    initBBOffset();
    bbIconMaxH 		= 0;
    showBBIcons_width = 0;
...
void CInfoViewer::getBBButtonInfo()
{
    bbButtonMaxH = 0;
    ...
    int MaxBr = (BoxEndX - 10) - (ChanInfoX + 10);
    bbButtonMaxX = ChanInfoX + 10;
    int br = 0, count = 0;
    for (int i = 0; i < CInfoViewer::BUTTON_MAX; i++)
    {
        {
...
void CInfoViewer::showBBButtons(bool paintFooter)
{
    if (!is_visible)
        return;
    int i;
...
void CInfoViewer::showBBIcons(const int modus, const std::string & icon)
{
    ...
    {
        frameBuffer->paintIcon(icon, bbIconInfo[modus].x - time_width, ChanNameY + 8,
...
{
    showBBIcons(CInfoViewer::ICON_UPDATE, show ? NEUTRINO_ICON_UPDATE_AVAIL : NEUTRINO_ICON_UPDATE_AVAIL_GREY);
}

void CInfoViewer::showIcon_Logo()
{
    showBBIcons(CInfoViewer::ICON_LOGO, NEUTRINO_ICON_LOGO);
}
...
void CInfoViewer::showIcon_DD()
{
    ...
    showBBIcons(CInfoViewer::ICON_DD, dd_icon);
}
...
void CInfoViewer::showIcon_RadioText(bool rt_available)
{
    ...
    showBBIcons(CInfoViewer::ICON_RT, rt_icon);
}
...
void CInfoViewer::showIcon_16_9()
{
    ...
    if ((aspectRatio == 0) || ( g_RemoteControl->current_PIDs.PIDs.vpid == 0 ) || (aspectRatio != videoDecoder->getAspectRatio()))
    {
        ...
        showBBIcons(CInfoViewer::ICON_16_9, (aspectRatio > 2) ? NEUTRINO_ICON_16_9 : NEUTRINO_ICON_16_9_GREY);
    }
}
...
void CInfoViewer::showIcon_Resolution()
{
    ...
    showBBIcons(CInfoViewer::ICON_RES, icon_name);
}
...
void CInfoViewer::showOne_CAIcon()
{
    ...
    showBBIcons(CInfoViewer::ICON_CA, sIcon);
}
...
void CInfoViewer::showBarHdd(int percent)
{
    if (is_visible)
    {
        hddscale->reset();
...
#include <time.h>
#include <math.h>
void CInfoViewer::show_clock(int posx,int posy,int dia)
{
...
{
    initBBOffset();
    int ca_width = BoxEndX - ChanInfoX;

    if (g_settings.infobar_casystem_frame)
    {
        if (ca_bar == NULL)
            ca_bar = new CComponentsShapeSquare(ChanInfoX + OFFSET_INNER_MID, BoxEndY, ca_width - 2*OFFSET_INNER_MID, bottom_bar_offset - OFFSET_INNER_MID, NULL, CC_SHADOW_ON, COL_INFOBAR_CASYSTEM_PLUS_2, COL_INFOBAR_CASYSTEM_PLUS_0);
    ...
    {
        int xcnt = (BoxEndX - ChanInfoX - (g_settings.infobar_casystem_frame ? 24 : 0)) / 4;
        ...
        for (int i = 0; i < xcnt; i++)
        {
            ...
            {
                frameBuffer->paintBoxRel((ChanInfoX + (g_settings.infobar_casystem_frame ? 14 : 2)) + i*4, BoxEndY + (g_settings.infobar_casystem_frame ? 4 : 2) + j*4, 2, 2, COL_INFOBAR_PLUS_1);
            }
        }
    }
#endif
