	...
	while (context && context->playback && context->playback->isPlaying)
	{
		...
		if (!isWaitingForFinish && (ffmpegStatus == 0))
		{
			...
			else if (subtitleTrack && (subtitleTrack->Id == pid))
			{
				...
				if (c->codec)
				{
					printf("**Subtitle PID : 0x%X  Codec : %s\n", pid, avcodec_get_name(c->codec_id));
					AVSubtitle sub;
			...
			switch (get_codecpar(stream)->codec_type)
			{
...
#if LIBAVCODEC_VERSION_INT >= AV_VERSION_INT(61, 0, 0)
							uint16_t number_of_channels = get_codecpar(stream)->ch_layout.nb_channels;
#else
				...
				case AVMEDIA_TYPE_SUBTITLE:
				{
					if (get_codecpar(stream)->codec_id != AV_CODEC_ID_SSA &&
						...
						get_codecpar(stream)->codec_id != AV_CODEC_ID_DVD_SUBTITLE &&
					...
					{
						printf("subtitle with not supported codec [%s]\n", avcodec_get_name((uint32_t)get_codecpar(stream)->codec_id));
