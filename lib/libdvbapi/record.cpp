...
#include "record.h"
#include "debug.h"
#define dvbapi_debug(args...) _dvbapi_debug(DVBAPI_DEBUG_RECORD, this, args)
#define dvbapi_info(args...) _dvbapi_info(DVBAPI_DEBUG_RECORD, this, args)
...
{
	dvbapi_info("%s: fd %d, vpid 0x%03x\n", __func__, fd, vpid);
	int i;
	...
	if (i != 0)
	{
		...
		errno = i;
		dvbapi_info("%s: error creating thread! (%m)\n", __func__);
		delete dmx;
		dmx = NULL;
		return false;
	}
...
bool cRecord::Stop(void)
{
	...
	if (exit_flag != RECORD_RUNNING)
		dvbapi_info("%s: status not RUNNING? (%d)\n", __func__, exit_flag);
	...
	if (file_fd != -1)
		...
		dvbapi_info("%s: file_fd not open??\n", __func__);
	file_fd = -1;
	return true;
}
...
void cRecord::RecordThread()
{
	...
	dvbapi_info("BUFSIZE=0x%x READSIZE=0x%x\n", bufsize, readsize);
	if (!buf)
	{
		exit_flag = RECORD_FAILED_MEMORY;
		dvbapi_info("%s: unable to allocate buffer! (out of memory)\n", __func__);
		if (failureCallback)
	...
	while (exit_flag == RECORD_RUNNING)
	{
		if (buf_pos < bufsize)
		{
			if (overflow_count)
			{
				dvbapi_info("%s: Overflow cleared after %d iterations\n", __func__, overflow_count);
				overflow_count = 0;
			}
		...
		else
		{
			...
			if (!(overflow_count % 10))
				dvbapi_info("%s: buffer full! Overflow? (%d)\n", __func__, ++overflow_count);
			state = REC_STATUS_SLOW;
		}
		r = aio_error(&a);
		if (r == EINPROGRESS)
		{
			dvbapi_debug("%s: aio in progress, free: %d\n", __func__, bufsize - buf_pos);
			continue;
		}
		...
		if (r < 0)
		{
			...
			dvbapi_debug("%s: aio_return = %d (%m)\n", __func__, r);
			break;
		}
		else
			dvbapi_debug("%s: aio_return = %d, free: %d\n", __func__, r, bufsize - buf_pos);
		if (posix_fadvise(file_fd, 0, 0, POSIX_FADV_DONTNEED))
			perror("posix_fadvise");
		if (queued)
		{
	...
	{
		dvbapi_debug("%s: run-out write, buf_pos %d\n", __func__, buf_pos);
		r = aio_error(&a);
		if (r == EINPROGRESS)
		{
			usleep(50000);
