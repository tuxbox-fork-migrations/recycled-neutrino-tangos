...
#define dvbapi_debug(args...) _dvbapi_debug(DVBAPI_DEBUG_VIDEO, this, args)
#define dvbapi_info(args...) _dvbapi_info(DVBAPI_DEBUG_VIDEO, this, args)
#define dvbapi_debug_c(args...) _dvbapi_debug(DVBAPI_DEBUG_VIDEO, NULL, args)
#define dvbapi_info_c(args...) _dvbapi_info(DVBAPI_DEBUG_VIDEO, NULL, args)
...
retry:
	if ((fd = open(VDEV[devnum], O_RDWR | O_CLOEXEC)) < 0)
	{
		...
		dvbapi_info("#%d: %s cannot open %s: %m, retries %d\n", devnum, __func__, VDEV[devnum], n);
	}
...
//	static const char *m[] = { "panscan", "letterbox", "bestfit", "nonlinear", "(unset)" };
	...
	dvbapi_debug("%s: a:%d m:%d  %s\n", __func__, aspect, mode, m[mo]);

	if (aspect > 3 || aspect == 0)
		dvbapi_info("%s: invalid aspect: %d\n", __func__, aspect);
	else if (aspect > 0) /* -1 == don't set */
	{
		dvbapi_debug("%s: /proc/stb/video/aspect -> %s\n", __func__, a[aspect]);
		n = proc_put("/proc/stb/video/aspect", a[aspect], strlen(a[aspect]));
		if (n < 0)
			dvbapi_info("%s: proc_put /proc/stb/video/aspect (%m)\n", __func__);
	}

	if (mode == -1)
		return 0;
#if BOXMODEL_OSMIO4K || BOXMODEL_OSMIO4KPLUS
	dvbapi_debug("%s: /proc/stb/video/policy2 -> %s\n", __func__, m[mo]);
	n = proc_put("/proc/stb/video/policy2", m[mo], strlen(m[mo]));
#else
	dvbapi_debug("%s: /proc/stb/video/policy -> %s\n", __func__, m[mo]);
...
int cVideo::getAspectRatio(void)
{
	...
	{
		dvbapi_info("%s: VIDEO_GET_SIZE %m\n", __func__);
		return -1;
	}
...
int cVideo::Start(void * /*PcrChannel*/, unsigned short /*PcrPid*/, unsigned short /*VideoPid*/, void * /*hChannel*/)
{
	...
	if (stillpicture)
	{
		dvbapi_info("%s: stillpicture == true, doing implicit StopPicture()\n", __func__);
		stillpicture = false;
		Stop(1);
	}
...
int cVideo::Stop(bool blank)
{
	...
	if (stillpicture)
	{
		dvbapi_debug("%s: stillpicture == true\n", __func__);
		return -1;
	}
...
int cVideo::GetVideoSystem(void)
{
	...
	dvbapi_info("%s: could not find '%s' mode, returning VIDEO_STD_720P50\n", __func__, current);
...
void cVideo::GetVideoSystemFormatName(cs_vs_format_t *format, int system)
{
	...
	{
		dvbapi_info("%s: invalid system %d\n", __func__, system);
...
void cVideo::SetVideoMode(analog_mode_t mode)
{
	...
	{
		dvbapi_debug("%s: non-SCART mode ignored\n", __func__);
		return;
	}
	const char *m;
	switch (mode)
...
bool cVideo::ShowPicture(const char *fname)
{
	...
	if (video_standby)
	{
		...
		dvbapi_info("%s: video_standby == true\n", __func__);
		return ret;
	}
	/* in movieplayer mode, fd is not opened */
	if (fd == -1)
	{
		dvbapi_info("%s: decoder not opened\n", __func__);
		return ret;
	}
...
void cVideo::Pig(int x, int y, int w, int h, int osd_w, int osd_h, int startx, int starty, int endx, int endy)
{
	...
	dvbapi_debug("#%d %s: x:%d y:%d w:%d h:%d ow:%d oh:%d\n", devnum, __func__, x, y, w, h, osd_w, osd_h);
	if (x == -1 && y == -1 && w == -1 && h == -1)
	{
		_w = xres;
		_h = yres;
	...
	dvbapi_debug("#%d %s: x:%d y:%d w:%d h:%d xr:%d yr:%d\n", devnum, __func__, _x, _y, _w, _h, xres, yres);
	sprintf(buffer, "%x", _x);
...
void cVideo::getPictureInfo(int &width, int &height, int &rate)
{
	...
	height = s.h;
	width = s.w;
	dvbapi_debug("#%d: %s: rate: %d, width: %d height: %d\n", devnum, __func__, rate, width, height);
}
...
int cVideo::SetStreamType(VIDEO_FORMAT type)
{
	...
	if (ioctl(fd, VIDEO_SET_STREAMTYPE, t) < 0)
		dvbapi_info("%s VIDEO_SET_STREAMTYPE(%d) failed: %m\n", __func__, t);
	return 0;
}
...
int64_t cVideo::GetPTS(void)
{
	...
	if (ioctl(fd, VIDEO_GET_PTS, &pts) < 0)
		dvbapi_info("%s: GET_PTS failed (%m)\n", __func__);
	return pts;
}

void cVideo::SetDemux(cDemux *)
{
	dvbapi_debug("#%d %s not implemented yet\n", devnum, __func__);
}
...
static bool swscale(unsigned char *src, unsigned char *dst, int sw, int sh, int dw, int dh, AVPixelFormat sfmt)
{
	...
	if (!scale)
	{
		dvbapi_info_c("%s: ERROR setting up SWS context\n", __func__);
		return ret;
	}
	...
	{
		dvbapi_info_c("%s: could not alloc sframe (%p) or dframe (%p)\n", __func__, sframe, dframe);
		ret = false;
	}
	...
	dvbapi_info_c("%s: %s scale %ix%i to %ix%i ,len %i\n", ret ? " " : "ERROR", __func__, sw, sh, dw, dh, len);
...
#define VDEC_PIXFMT AV_PIX_FMT_BGR24
	...
	dvbapi_info("%s: out_data 0x%p xres %d yres %d vid %d osd %d scale %d\n",
