...
#define dvbapi_debug(args...) _dvbapi_debug(DVBAPI_DEBUG_PLAYBACK, this, args)
#define dvbapi_info(args...) _dvbapi_info(DVBAPI_DEBUG_PLAYBACK, this, args)
#define dvbapi_debug_c(args...) _dvbapi_debug(DVBAPI_DEBUG_PLAYBACK, NULL, args)
#define dvbapi_info_c(args...) _dvbapi_info(DVBAPI_DEBUG_PLAYBACK, NULL, args)
...
void processMpegTsSection(GstMpegtsSection *section)
{
	...
	if (section->section_type == GST_MPEGTS_SECTION_PMT)
	{
		...
		for (guint i = 0; i < pmt->streams->len; ++i)
		{
			...
			{
				dvbapi_info_c("%s:%s Audio Stream pid: %d\n", FILENAME, __FUNCTION__, stream->pid);
...
GstBusSyncReply Gst_bus_call(GstBus *bus, GstMessage *msg, gpointer user_data)
{
	...
	switch (GST_MESSAGE_TYPE(msg))
	{
		...
		case GST_MESSAGE_ERROR:
		{
			...
			dvbapi_info_c("%s:%s - GST_MESSAGE_ERROR: %s (%i) from %s\n", FILENAME, __FUNCTION__, err->message, err->code, sourceName);
			if (err->domain == GST_STREAM_ERROR)
			{
				if (err->code == GST_STREAM_ERROR_CODEC_NOT_FOUND)
				{
					if (g_strrstr(sourceName, "videosink"))
						dvbapi_info_c("%s:%s - GST_MESSAGE_ERROR: videosink\n", FILENAME, __FUNCTION__);   //FIXME: how sdvbapil playback handle this event???
					else if (g_strrstr(sourceName, "audiosink"))
						dvbapi_info_c("%s:%s - GST_MESSAGE_ERROR: audioSink\n", FILENAME, __FUNCTION__);   //FIXME: how sdvbapil playback handle this event???
				}
			}
			g_error_free(err);
			if (sourceName)
		...
		case GST_MESSAGE_INFO:
		{
			...
			if (inf->domain == GST_STREAM_ERROR && inf->code == GST_STREAM_ERROR_DECODE)
			{
				gchar *sourceName = gst_object_get_name(source);
				if (g_strrstr(sourceName, "videosink"))
					dvbapi_info_c("%s:%s - GST_MESSAGE_INFO: videosink\n", FILENAME, __FUNCTION__);   //FIXME: how sdvbapil playback handle this event???
				if (sourceName)
					g_free(sourceName);
		...
		case GST_MESSAGE_TAG:
		{
			...
			if (gv_image)
			{
				...
				if (fd >= 0)
				{
					...
					close(fd);
					dvbapi_info_c("%s:%s - /tmp/.id3coverart %d bytes written\n", FILENAME, __FUNCTION__, ret);
				}
			}
			if (tags)
				gst_tag_list_unref(tags);
			dvbapi_debug_c("%s:%s - GST_MESSAGE_INFO: update info tags\n", FILENAME, __FUNCTION__);   //FIXME: how sdvbapil playback handle this event???
			break;
		}
			...
			switch (transition)
			{
				...
				case GST_STATE_CHANGE_READY_TO_PAUSED:
				{
					...
					if (children && gst_iterator_find_custom(children, (GCompareFunc)match_sinktype, &r, (gpointer)"GstDVBAudioSink"))
					{
						...
						g_value_unset(&r);
						dvbapi_info_c("%s %s - audio sink created\n", FILENAME, __FUNCTION__);
					}
					...
					if (children && gst_iterator_find_custom(children, (GCompareFunc)match_sinktype, &r, (gpointer)"GstDVBVideoSink"))
					{
						...
						g_value_unset(&r);
						dvbapi_info_c("%s %s - video sink created\n", FILENAME, __FUNCTION__);
					}
					gst_iterator_free(children);
...
cPlayback::cPlayback(int num)
{
	...
	dvbapi_info("%s:%s - This program is linked against GStreamer %d.%d.%d %s\n",
		FILENAME, __FUNCTION__,
...
void cPlayback::Close(void)
{
	...
	if (m_gst_playbin)
	{
		...
		dvbapi_info("%s:%s - GST bus handler closed\n", FILENAME, __FUNCTION__);
	}
	...
	if (m_gst_playbin)
	{
		if (audioSink)
		{
			...
			dvbapi_info("%s:%s - GST audio Sink closed\n", FILENAME, __FUNCTION__);
		}
		...
		if (videoSink)
		{
			...
			dvbapi_info("%s:%s - GST video Sink closed\n", FILENAME, __FUNCTION__);
		}

		// unref m_gst_playbin
		gst_object_unref(GST_OBJECT(m_gst_playbin));
		dvbapi_info("%s:%s - GST playbin closed\n", FILENAME, __FUNCTION__);
...
bool cPlayback::Start(char *filename, int /*vpid*/, int /*vtype*/, int /*apid*/, int /*ac3*/, int /*duration*/, std::string headers)
{
	...
	{
		dvbapi_info("%s:%s - failed to create GStreamer pipeline!, sorry we can not play\n", FILENAME, __FUNCTION__);
		playing = false;
...
{
	dvbapi_info("%s: pos %d abs %d playing %d\n", __func__, position, absolute, playing);
